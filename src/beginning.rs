...
pub mod initial_state {
	...
	use crate::{Execution, PUT_UP, SLAVE_PKG};
			...
			*mutex.lock().unwrap() = true;
			...
			if *y.wait(x.lock().unwrap()).unwrap() {
				if PUT_UP.get().unwrap().drive.master.cluster_enabled {
					Colour::error_display("node_build", node_build().await);
				} else {}
			}
			Ok(())
		})], "initial")).run_async().await?;
		Ok(())
	}
		...
		for i in PUT_UP.get().unwrap().drive.slave.iter() {
			x.push(spawn(async move {
				...
				if PUT_UP.get().unwrap().drive.master.scp_build {
					t = format!("ls -la {}", PUT_UP.get().unwrap().data.storage.node_data.to_str().unwrap())
				}
				for (x, (_a, b)) in i.shell(vec![
					format!("chmod -R 777 {}", PUT_UP.get().unwrap().data.storage.node_data.to_str().unwrap()).as_str(),
					//驱动执行
					format!("cd {} && {}", PUT_UP.get().unwrap().data.storage.node_data.to_str().unwrap(), PUT_UP.get().unwrap().data.storage.node_service_path.to_str().unwrap()).as_str(),
					&t,
				]).unwrap_or_else(|_| {
					println!("{}", Colour::Debug.table(Information {
						list: ["Node", "PossibleError"],
						data: [[&i.node_name, "TimeOut"], [&i.node_name, "FileConfigurationError"], [&i.node_name, "SessionMisconfiguration"]],
					}));
					HashMap::default()
				}).iter().enumerate() {
					if x != 0 {
						...
						println!("{}", Colour::Output.table(Information { list: ["Node", "Local_Path", "Remote_Path"], data: [[&i.node_name, SLAVE_PKG, x.to_str().unwrap()]] }));
					}
				}
